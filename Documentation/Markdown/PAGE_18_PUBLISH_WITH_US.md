# PAGE_18_PUBLISH_WITH_US

**URL:** `https://securityblogs.com.au/publish-with-us/`
**Airtable table:** `SB · 18 · Publish With Us Hub`
**Page kind:** hub
**Sections in spec:** 4
...
Hub linking to the 7 monetisation surfaces (guest posting, sponsored, advertise, press release, backlinks, product promo, pricing).
...
|---|---|---|
| 1 | Hero | hero |
| 2 | Tilt cards (7 options) | grid (server) |
| 3 | CTA band | cta-band |
| 4 | FAQs | faqs |
...
| Table | Use |
|---|---|
| `SB · 18 · Publish With Us Hub` | Per-section spec for `/publish-with-us/` (one row per section) |
...
|---|---|---|---|
| `GET /api/pages?where[slug][equals]=publish-with-us` | GET | ISR 60s | Page record + modules |
...
| Component | Purpose |
|---|---|
| `app/publish-with-us/page.tsx` | Server component — fetches CMS data, renders |
| `app/publish-with-us/TiltCard.tsx` | Server component — list rendering |
...
- **Copy / hero text:** Low impact. Edit the CMS record (or `app/publish-with-us/page.tsx` if still hardcoded). Wait for ISR.
...
*End of PAGE_18_PUBLISH_WITH_US.md*
