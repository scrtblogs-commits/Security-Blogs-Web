...
**URL:** `https://securityblogs.com.au/free-tools/`
**Airtable table:** `SB · 31 · Free Tools Hub`
**Page kind:** tool
**Sections in spec:** 5
...
Free tools collection: Visibility Checker (form), AI Visibility Checklist (download), Guides tabs.
...
|---|---|---|
| 1 | Hero | hero |
| 2 | Visibility Checker widget | client widget |
| 3 | Checklist download | rich-text |
| 4 | Guides tabs | tabs (client) |
| 5 | CTA band | cta-band |
...
| Table | Use |
|---|---|
| `SB · 31 · Free Tools Hub` | Per-section spec for `/free-tools/` (one row per section) |
...
| Component | Purpose |
|---|---|
| `app/free-tools/page.tsx` | Server component — fetches CMS data, renders |
| `app/free-tools/VisibilityChecker.tsx` | Component used on this page |
| `app/free-tools/Checklist.tsx` | Component used on this page |
| `app/free-tools/GuidesTabs.tsx` | Client component — interactive tabs |
...
- **Copy / hero text:** Low impact. Edit the CMS record (or `app/free-tools/page.tsx` if still hardcoded). Wait for ISR.
...
- **Visibility Checker** — posts to `/api/leads` (see Request/Response schemas)
- **Checklist email gate** — posts to `/api/leads` (see Request/Response schemas)

---

*Notes:* Visibility Checker posts to /api/leads with source='visibility-checker' and meta.domain + meta.score.

*End of PAGE_31_FREE_TOOLS.md*
