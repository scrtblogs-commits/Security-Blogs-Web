# PAGE_20_BACKLINK_PACKAGES

**URL:** `https://securityblogs.com.au/publish-with-us/backlink-packages/`
**Airtable table:** `SB · 20 · Sponsored Editorial Placements`
**Page kind:** landing
**Sections in spec:** 4
...
Sponsored editorial placements (was 'backlink packages' but renamed for compliance).
...
| Table | Use |
|---|---|
| `SB · 20 · Sponsored Editorial Placements` | Per-section spec for `/publish-with-us/backlink-packages/` (one row per section) |
...
|---|---|---|---|
| `GET /api/pages?where[slug][equals]=backlink-packages` | GET | ISR 60s | Page record + modules |
...
```json
{
  "docs": [{
    "id": "rec…",
    "slug": "backlink-packages",
    "title": "Sponsored Editorial Placements",
    "modules": [
...
| Component | Purpose |
|---|---|
| `app/publish-with-us/backlink-packages/page.tsx` | Server component — fetches CMS data, renders |
...
2. Navigate to `/publish-with-us/backlink-packages/`
...
- **Copy / hero text:** Low impact. Edit the CMS record (or `app/publish-with-us/backlink-packages/page.tsx` if still hardcoded). Wait for ISR.
...
- **Inquiry form** — posts to `/api/leads` (see Request/Response schemas)

---

*Notes:* Avoid using DA50+/DA60+/DA65+ phrasing per legal review.

*End of PAGE_20_BACKLINK_PACKAGES.md*
