...
**URL:** `https://securityblogs.com.au/career/`
**Airtable table:** `SB · 32 · Career`
**Page kind:** form
**Sections in spec:** 4
...
Job openings + applicant submission with CV upload.
...
|---|---|---|
| 1 | Hero | hero |
| 2 | Open roles grid | grid (server) |
| 3 | Culture strip | rich-text |
| 4 | Application form | form (client component) |

## Airtable Tables

| Table | Use |
|---|---|
| `SB · 32 · Career` | Per-section spec for `/career/` (one row per section) |
...
```json
{
  ...
  "source": "career",
  "meta": { "position": "Senior Engineer", "cv_url": "[file: cv.pdf]" },
...
| Component | Purpose |
|---|---|
| `app/career/page.tsx` | Server component — fetches CMS data, renders |
| `app/career/ApplicationForm.tsx` | Client component — handles submit, calls submitForm() |
...
| `app/api/leads/upload/route.ts` (Phase C.2) | Multipart endpoint for CV uploads |
...
- **Copy / hero text:** Low impact. Edit the CMS record (or `app/career/page.tsx` if still hardcoded). Wait for ISR.
...
- **Job application** — posts to `/api/leads` (see Request/Response schemas)
