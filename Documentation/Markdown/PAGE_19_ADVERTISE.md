...
**URL:** `https://securityblogs.com.au/publish-with-us/advertise/`
**Airtable table:** `SB · 19 · Advertise With Us`
**Page kind:** landing
**Sections in spec:** 4
...
Advertise on SecurityBlogs — display ads, sponsored sections, newsletter sponsorship.
...
|---|---|---|
| 1 | Hero | hero |
| 2 | Ad inventory | rich-text |
| 3 | Pricing grid | grid (server) |
| 4 | CTA band | cta-band |
...
| Table | Use |
|---|---|
| `SB · 19 · Advertise With Us` | Per-section spec for `/publish-with-us/advertise/` (one row per section) |
...
|---|---|---|---|
| `GET /api/pages?where[slug][equals]=advertise` | GET | ISR 60s | Page record + modules |
...
| Component | Purpose |
|---|---|
| `app/publish-with-us/advertise/page.tsx` | Server component — fetches CMS data, renders |
...
2. Navigate to `/publish-with-us/advertise/`
...
- **Copy / hero text:** Low impact. Edit the CMS record (or `app/publish-with-us/advertise/page.tsx` if still hardcoded). Wait for ISR.
