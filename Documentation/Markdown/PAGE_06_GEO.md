...
**URL:** `https://securityblogs.com.au/services/geo/`
**Airtable table:** `SB · 06 · GEO (Generative Engine Optimisation)`
**Page kind:** detail
**Sections in spec:** 7
...
Service detail for GEO + Australia capital-city map widget showing pulsing pins.
...
|---|---|---|
| 1 | Hero | hero |
| 2 | Australia GEO map | rich-text |
| 3 | Capabilities | capabilities |
| 4 | Process | process-steps |
| 5 | Stats | stats |
| 6 | FAQs | faqs |
| 7 | CTA band | cta-band |
...
| Table | Use |
|---|---|
| `SB · 06 · GEO (Generative Engine Optimisation)` | Per-section spec for `/services/geo/` (one row per section) |
...
|---|---|---|---|
| `GET /api/services?where[slug][equals]=geo` | GET | ISR 60s | Service record |
...
### `GET /api/services?where[slug][equals]=geo`
...
| Component | Purpose |
|---|---|
| `app/services/geo/page.tsx` | Server component — fetches CMS data, renders |
| `app/services/geo/AustraliaMap.tsx (or similar)` | Client component — Mapbox/GMaps integration |
...
| `NEXT_PUBLIC_MAPBOX_TOKEN` | Yes | Mapbox tiles |
...
- Mapbox token must be URL-restricted in Mapbox dashboard.

## Testing Procedures

### Local smoke
1. `pnpm dev` (marketing site at `http://localhost:3000`)
2. Navigate to `/services/geo/`
...
| Map tiles missing | Mapbox token invalid or URL not allowlisted | Check Mapbox dashboard referrer rules |

## Change Impact Analysis

**If you change…**

- **Copy / hero text:** Low impact. Edit the CMS record (or `app/services/geo/page.tsx` if still hardcoded). Wait for ISR.
