...
const STEPS = [
  { step: '01', tag: 'FOUNDATION',  title: 'SEO Audit & Keyword Research', color: '#1e5fe0', glow: 'rgba(30,95,224,0.45)'   },
  { step: '02', tag: 'OPTIMISE',    title: 'Technical Fixes & On-Page',    color: '#1e9e75', glow: 'rgba(30,158,117,0.45)' },
  { step: '03', tag: 'AUTHORITY',   title: 'Content & Link Building',      color: '#9b5cf6', glow: 'rgba(155,92,246,0.45)' },
  { step: '04', tag: 'RESULTS',     title: 'Monitor & Scale',              color: '#f59e0b', glow: 'rgba(245,158,11,0.45)' },
]
  ...
  return (
    <div ref={outerRef} style={{ height: `${total * 100}vh`, position: 'relative' }}>
      <div style={{
        ...
        {/* Ambient glow orb — colour shifts per active card */}
        <GlowOrb floatIdx={floatIdx} />
        ...
        <div style={{ textAlign: 'center', paddingTop: 42, flexShrink: 0, position: 'relative', zIndex: 2 }}>
          ...
          <div style={{ position: 'relative', height: 34, marginTop: 6 }}>
            {STEPS.map((s, i) => <ActiveTitle key={i} index={i} floatIdx={floatIdx} step={s} />)}
          </div>
        </div>
        ...
        <motion.div style={{
          ...
          {STEPS.map((s, i) => <SceneCard key={i} index={i} floatIdx={floatIdx} data={s} />)}
        </motion.div>
        ...
        <div style={{ position: 'absolute', bottom: 26, left: 0, right: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, zIndex: 2 }}>
          <div style={{ display: 'flex', gap: 10 }}>
            {STEPS.map((s, i) => <StepPill key={i} index={i} floatIdx={floatIdx} step={s} />)}
          </div>
          <motion.div
          ...
          >
            ...
            <motion.span animate={{ x: [0, 6, 0] }} transition={{ duration: 1.3, repeat: Infinity }} style={{ color: 'rgba(120,160,255,0.5)', fontSize: 13 }}>→</motion.span>
          </motion.div>
        </div>
      </div>
    </div>
...
/* ─── Ambient colour orb ─── */
function GlowOrb({ floatIdx }: { floatIdx: MotionValue<number> }) {
  const colors = STEPS.map(s => s.glow)
  const [col, setCol] = useState(colors[0])
  useEffect(() => {
    const unsub = floatIdx.on('change', fi => {
...
/* ─── Active title cross-fade ─── */
function ActiveTitle({ index, floatIdx, step }: { index: number; floatIdx: MotionValue<number>; step: typeof STEPS[0] }) {
  const opacity = useTransform(floatIdx, fi => Math.abs(index - fi) < 0.45 ? 1 : 0)
  const y       = useTransform(floatIdx, fi => Math.abs(index - fi) < 0.45 ? 0 : 8)
  return (
    <motion.div style={{
...
/* ─── Step pill ─── */
function StepPill({ index, floatIdx, step }: { index: number; floatIdx: MotionValue<number>; step: typeof STEPS[0] }) {
  const [active, setActive] = useState(index === 0)
  useEffect(() => {
    const unsub = floatIdx.on('change', fi => setActive(Math.abs(index - fi) < 0.45))
    return unsub
  }, [floatIdx, index])
  return (
    <div style={{
...
/* ─── Scene card wrapper ─── */
function SceneCard({ index, floatIdx, data }: { index: number; floatIdx: MotionValue<number>; data: typeof STEPS[0] }) {
  const [active, setActive] = useState(index === 0)

  useEffect(() => {
    const unsub = floatIdx.on('change', fi => setActive(Math.abs(index - fi) < 0.4))
    return unsub
  }, [floatIdx, index])

  const scale   = useTransform(floatIdx, fi => { const d = Math.abs(index - fi); return d < 0.05 ? 1 : d < 1 ? 1 - d * 0.06 : 0.88 })
  const opacity = useTransform(floatIdx, fi => { const d = Math.abs(index - fi); return d < 0.05 ? 1 : d < 1.5 ? 1 - d * 0.28 : 0.22 })
  ...
  return (
    <motion.div style={{ scale, opacity, flexShrink: 0 }}>
      <motion.div style={{
        ...
        <SceneComp active={active} color={data.color} />
      </motion.div>
    </motion.div>
  )
}
